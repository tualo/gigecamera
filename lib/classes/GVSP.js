...
GVSP.prototype.start=function(port){
  ...
  me.server.on("message", function (msg, rinfo) {
    ...
    if (message.format==='DATA_LEADER'){
      ...
      /*
      console.log(message.format,message.block_id,message.packet_id,message.payload.length);
      ...
      if (me.image!==null){
        ...
        if(me.image.lastPacket+1!=message.packet_id){
          var cmd = new COMMANDS.PACKETRESEND.PACKETRESEND();
          cmd.set(message.block_id,me.image.lastPacket,message.packet_id);
          me.gvcp.force(cmd);
        }

        me.image.set(message);
      }
      */
