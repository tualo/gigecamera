...
Device.prototype.server = function(){
  ...
  data.setBit(30,false); //CA simple access
  data.setBit(31,true); //EA exclusive Access
  ...
  me.writeRegister('0x0A00',data.readUIntBE(0,4),function(err,response){
    if (err){
      ...
      data[2]=178;
      data[3]=45;
      me.writeRegister( '0x'+Number(0x0D18 + 0x40 * iface).toString(16),data,function(err,response){
        if (err){
          throw new Error(err);
        }else{
          var data = new Buffer(4);
          var iface = 0;
          data.fill(0);
          data.writeUIntBE( server.port,2,2);
...
Device.prototype.writeRegister = function(address,data,callback,hint){
  var me = this;
