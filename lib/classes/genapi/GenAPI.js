    ...
    if (stack.length>1){
      ...
      if (
        ...
        if ('RegisterDescription'!==stack[stack.length-2].tag){
          ...
          if ('Category' === stack[stack.length-2].tag){
            categoryName = stack[stack.length-2].attr['Name'];

            if (typeof me.categories[categoryName]==='undefined'){
              me.categories[categoryName] = new Category(me,categoryName);
            }

            if (typeof me.categories[categoryName][stack[stack.length-1].tag]==='function'){
              if ('pFeature' === stack[stack.length-1].tag){
                me.features[stack[stack.length-1].value]=categoryName;
              }
              me.categories[categoryName][stack[stack.length-1].tag](stack[stack.length-1].value)
            }else{
              console.log('GenAPI',stack[stack.length-1].tag,' not supported');
            }
            //process.exit();
          }else if ('Enumeration' === stack[stack.length-2].tag){

            featureName = stack[stack.length-2].attr['Name'];
            if (typeof me.features[featureName]==='string'){
              categoryName = me.features[featureName];
              enumObject = me.categories[categoryName].getOrCreateEnummeration(featureName);
              if (typeof enumObject[stack[stack.length-1].tag]==='function'){
                enumObject[stack[stack.length-1].tag](stack[stack.length-1].value);
              }else{
                console.log('GenAPI','Enumeration.'+stack[stack.length-1].tag,' not supported');
                process.exit();
              }
            }else{
              console.log('GenAPI','Feature',featureName,'is not defined');
              process.exit();
            }

          }else if ('EnumEntry' === stack[stack.length-2].tag){

            entry = enumObject.getOrCreateEnumEntry(stack[stack.length-2].attr['Name']);
            if (typeof entry[stack[stack.length-1].tag]==='function'){
              entry[stack[stack.length-1].tag](stack[stack.length-1].value);
            }else{
              console.log('GenAPI','EnumEntry.'+stack[stack.length-1].tag,' not supported');
              process.exit();
            }

          }else{

            //var categoryName = stack[stack.length-2].attr['Name'];
