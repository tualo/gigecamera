    ...
    me.writeRegister('0x0A00',data.readUIntBE(0,4),function(err,response){
      if (err){
        ...
        me.readRegister( '0x0A14',4,function(err,response){
          ...
          console.log('control ip',response,'0x'+Number(0x0D18 + 0x40 * iface).toString(16));

        me.writeRegister( '0x'+Number(0x0D18 + 0x40 * iface).toString(16),response.readUIntBE(0,4),function(err,response){
          if (err){
            throw new Error(err);
          }else{
