...
exports.imageing = function(packetSize,prefix,broadcast,exposure){
  ...
  var liveImage = new opencv.Matrix(maxPreviewHeight,width,opencv.Constants.CV_8UC1);
  var buffer = new Buffer(maxPreviewHeight*width);

  buffer.fill(30);
  liveImage.put(buffer);
  ...
  device.on('initializedServer',function(server){
    ...
    server.on('image',function(img){
      ...
      var oLiveBuffer = liveImage.getData();
      var nLiveBuffer = new Buffer(oLiveBuffer.length);

      nLiveBuffer.fill(0);

      oLiveBuffer.copy(nLiveBuffer,0,shift,oLiveBuffer.length-img.buffer.length);

      console.log(img.id);
      img.buffer.copy(nLiveBuffer,oLiveBuffer.length-img.buffer.length-img.width,0,img.buffer.length);

      //img.buffer.copy(nLiveBuffer,oLiveBuffer.length-1024,0,img.buffer.length);

      liveImage.put(nLiveBuffer);
      helper++;
      if (helper===10){
        helper = 0;
        live.show(liveImage);
        live.blockingWaitKey(0, 50);
      }
        ...
        if (getAVG){
          ...
          if ( (sum/img.buffer.length) > start_avg ){
            ...
            if (!isimageing){
              ...
              isimageing = true;
              console.log('start image',blockOffset);
              offset=0;
            }
            ...
            if (img.id < blockOffset){
              console.log('getting older block');
            }else{
