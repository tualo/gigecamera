...
var GVCP = function() {
  ...
  me.broadcastAddress = '255.255.255.255';
  events.EventEmitter.call(this);

  me.client = dgram.createSocket("udp4");
  me.client.on('close', function(){
    error('GCVP','client','closed');
  } );
  me.client.on('error', function(e){
    error('GCVP','client','error'+e);
  } );
...
GVCP.prototype.discover = function(broadcastAddress){
  var me = this;
  var cmd = new COMMANDS.DISCOVERY.DISCOVERY();
  if (typeof broadcastAddress!=='undefined'){
    me.broadcastAddress = broadcastAddress;
  }
  console.log(me.broadcastAddress);
...
GVCP.prototype.force = function(cmd){
  var me = this,message;
  cmd.req_id = me.nextID();
  message = cmd.toBuffer();
  //me.emit('packetSend',cmd.req_id);
  me.client.send(message, 0, message.length, 3956, me.ip, function(err, bytes) {
    if (err){
      console.log(err);
    }else{
      //console.log('*',me.ip,cmd);
      me.emit('packetSend',cmd.req_id,cmd.name);
    }
  });
}
